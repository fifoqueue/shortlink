...
<svelte:head>
  <title>{text.auth.forgotPasswordTitle} · {data.siteName}</title>
</svelte:head>
...
>
  ...
  <section>
    ...
    <h1>{text.auth.forgotPasswordTitle}</h1>
    <p class="muted">{text.auth.forgotPasswordDescription}</p>
    ...
    {:else}
      <form method="POST" action="?/request" use:enhance>
        ...
        <button type="submit">{text.auth.sendPasswordResetEmail}</button>
      </form>
    {/if}
