...
>
  ...
  <section>
    ...
    <div class="grid form-grid balanced">
      ...
      <label class="wide">
        {t('admin.description')}
        <Textarea name="description" rows={3} value={group.description} />
      </label>
      <div class="wide">
        <ToggleField
          name="enabled"
  ...
  <section>
    ...
    <Tabs.Root
      value={autoAssignTab}
      onValueChange={(value) => (autoAssignTab = value as AutoAssignTab)}
    >
      <Tabs.List class="flex-wrap" aria-label={t('admin.automaticAssignment')}>
        {#each autoAssignTabs as tab (tab[0])}<Tabs.Trigger value={tab[0]}
            >{t(tab[1])}</Tabs.Trigger
          >{/each}
      </Tabs.List>
      <Tabs.Content
        value="email"
        class="auto-assign-tab-panel grid gap-4 data-[state=inactive]:hidden"
      >
        <label>
          {t('admin.autoAssignEmailPatterns')}
          <small>{t('admin.autoAssignEmailPatternsHelp')}</small>
          <Textarea
            name="autoAssign.emailPatterns"
            rows={4}
            value={emailPatternCondition(group)}
          />
        </label>
      </Tabs.Content>
      <Tabs.Content
  ...
  <Tabs.Root
    value={activeTab}
    onValueChange={(value) => (activeTab = value as PermissionTab)}
  >
    <Tabs.List
      class="flex-wrap"
      aria-label={t('admin.permissionGroupSettings')}
    >
      <Tabs.Trigger value="links">{t('admin.links')}</Tabs.Trigger>
      <Tabs.Trigger value="admin">{t('admin.administrator')}</Tabs.Trigger>
      <Tabs.Trigger value="auth">{t('admin.authentication')}</Tabs.Trigger>
      <Tabs.Trigger value="api">{t('admin.api')}</Tabs.Trigger>
    </Tabs.List>
    <Tabs.Content
      value="links"
      class="tab-panel grid gap-4 data-[state=inactive]:hidden"
    >
      <section>
        ...
        <div class="grid form-grid balanced dense-grid override-grid">
          ...
          <div class="override-control">
            ...
            <Textarea
              name="allowedDomains"
              rows={4}
              value={group.rules.links.domains?.join('\n') ?? ''}
            />
        ...
        <div class="grid form-grid balanced dense-grid">
          ...
          <label class="wide">
            ...
            <Textarea
              name="adminPlugins"
              rows={4}
              value={group.rules.admin.plugins.join('\n')}
            />
          </label>
        </div>
      </section>
